    ...
    <div className="py-10 md:py-16 flex flex-col w-full animate-fade-in px-6 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-text-secondary font-bold text-lg">Tạo hoặc tham gia không gian chung cùng bạn bè</p>
        </div>
      ...
      {showCreate && (
        <div className="bg-card-bg/70 backdrop-blur-xl p-8 rounded-[35px] border border-border-primary shadow-2xl shadow-pink-500/5 mb-12 animate-slide-up max-w-2xl mx-auto w-full">
          <div className="flex flex-col gap-6">
            <h3 className="text-xl font-black text-text-primary">Khởi tạo nhóm mới</h3>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowCreate(false)} className="px-6 py-2 font-bold text-text-muted hover:text-text-secondary">Hủy</button>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {groups.length === 0 && !showCreate && (
          <div className="col-span-full py-32 flex flex-col items-center justify-center bg-bg-primary/50 rounded-[40px] border-2 border-dashed border-border-primary text-center">
            ...
            <p className="text-text-muted font-bold text-lg">Chưa có nhóm nào được tạo</p>
          </div>
        )}
        {groups.map(g => (
          <GroupCard
        ...
        <div onClick={handleCloseJoin} className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[10000] p-6 animate-fade-in">
          <div onClick={e => e.stopPropagation()} className="bg-card-bg rounded-[40px] w-full max-w-[440px] p-10 shadow-2xl animate-slide-up flex flex-col gap-8 border border-border-primary">
            <div className="text-center">
              <h3 className="text-3xl font-black text-text-primary tracking-tight mb-2">Tham gia nhóm</h3>
              <p className="text-text-muted font-medium">Nhập mã mời 6 ký tự để gia nhập</p>
            </div>
            <Input
              value={joinCode}
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={handleCloseJoin} className="w-full h-12 text-text-muted font-bold hover:text-text-secondary">Để sau</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editModal.visible && (
        <div onClick={handleCloseEdit} className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[10000] p-6 animate-fade-in">
          <div onClick={e => e.stopPropagation()} className="bg-card-bg rounded-[40px] w-full max-w-[480px] p-10 shadow-2xl animate-slide-up flex flex-col gap-8 border border-border-primary">
            <div className="text-center">
              <h3 className="text-3xl font-black text-text-primary tracking-tight mb-2">Chỉnh sửa nhóm</h3>
              <p className="text-text-muted font-medium">Cập nhật thông tin không gian chung</p>
            </div>

            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-black uppercase tracking-wider text-text-muted ml-1">Tên nhóm</label>
                <Input
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-black uppercase tracking-wider text-text-muted ml-1">Mô tả</label>
                <Input
                  as="textarea"
                  rows={3}
