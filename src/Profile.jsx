...
export default function Profile({ userProfile, onClose, onUpdate, isReadOnly = false }) {
    ...
    if (!userProfile) {
        return (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-center justify-center z-[9999] p-4 animate-fade-in" onClick={onClose}>
                <div className="bg-white/95 backdrop-blur-3xl p-10 rounded-[40px] shadow-2xl flex flex-col items-center gap-6 border border-white">
                    <div className="w-12 h-12 border-[5px] border-pink-500 border-t-transparent rounded-full animate-spin" />
                    <div className="flex flex-col items-center gap-1">
                        <p className="text-gray-900 font-black text-lg">Đang tải hồ sơ...</p>
                        <p className="text-gray-400 font-bold text-xs uppercase tracking-widest">Vui lòng đợi trong giây lát</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-center justify-center z-[9999] p-4 animate-fade-in" onClick={() => !isActiveAction && handleRequestClose()}>
            <div
                style={{ backgroundColor: activeTheme?.backgroundColor || "#ffffff" }}
                className="bg-white/95 backdrop-blur-3xl w-full max-w-[380px] rounded-[40px] overflow-hidden relative animate-slide-up flex flex-col font-sans border border-white/20 transition-all duration-500"
                            ...
                            <div className="flex items-center justify-center p-5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[14px] text-gray-800 font-bold flex items-center justify-center">
                        ...
                        <div className="flex flex-col gap-4 max-h-[50vh] overflow-y-auto custom-scrollbar px-1 -mx-1">
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <Input value={displayName} onChange={e => setDisplayName(e.target.value)} className="!bg-gray-50/50 !border-gray-100 !text-gray-900 !h-12 !rounded-2xl focus:!bg-white focus:!ring-2 focus:!ring-pink-100 font-bold" />
                                </div>
                                <div>
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Tên đăng nhập (Username)</label>
                                    <Input value={username} onChange={e => setUsername(e.target.value)} className="!bg-gray-50/50 !border-gray-100 !text-gray-900 !h-12 !rounded-2xl focus:!bg-white focus:!ring-2 focus:!ring-pink-100 font-bold" />
                                </div>
                                <div>
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Đang làm gì thế?</label>
                                    <Input value={customStatus} onChange={e => setCustomStatus(e.target.value)} placeholder="Suy nghĩ gì đó..." className="!bg-gray-50/50 !border-gray-100 !text-gray-900 !h-12 !rounded-2xl focus:!bg-white focus:!ring-2 focus:!ring-pink-100 font-bold" />
                                </div>
                                <div>
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Giới thiệu bản thân</label>
                                    <Input as="textarea" rows={3} value={bio} onChange={e => setBio(e.target.value)} className="!bg-gray-50/50 !border-gray-100 !text-gray-900 !rounded-2xl focus:!bg-white focus:!ring-2 focus:!ring-pink-100 font-medium" />
                                </div>
                                <div>
                                    ...
                                    <button
                                        type="button"
                                        onClick={openStatusSelector}
                                        className="w-full bg-gray-50 border border-gray-100 text-gray-900 h-14 rounded-2xl px-5 flex items-center justify-between outline-none focus:ring-2 focus:ring-pink-100 font-bold transition-all hover:bg-white"
                                    >
                                        <div className="flex items-center gap-3">
                                        ...
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transition-transform duration-300 ${selectorAnchor ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9" /></svg>
                                    </button>
                                </div>

                                {/* THEME SELECTION */}
                                <div className="pt-2">
                                    <label className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-3 block">Giao diện hồ sơ</label>

                                    <div className="space-y-5">
                                        {/* Background Color Picker */}
                                        <div className="flex flex-col gap-3">
                                            <span className="text-[10px] font-bold text-gray-400 uppercase">Màu nền hồ sơ</span>
                                            <div className="flex items-center gap-4 bg-white/50 p-3 rounded-2xl border border-white/40">
                                                <div className="relative group">
                                                    <div
                                                        style={{ backgroundColor: activeTheme.backgroundColor }}
                                                        className="w-12 h-12 rounded-xl shadow-lg cursor-pointer border-2 border-white flex items-center justify-center transition-transform hover:scale-105 active:scale-95 overflow-hidden"
                                                ...
                                                <div className="flex flex-col flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[13px] font-black text-gray-800 tracking-tight uppercase">{activeTheme.backgroundColor}</span>
                                                        <button
                                                    ...
                                                    <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-tighter">Chọn màu nền bất kỳ</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Colors */}
                                        <div className="flex flex-col gap-3">
                                            <span className="text-[10px] font-bold text-gray-400 uppercase">Màu chủ đạo (Accent)</span>
                                            <div className="flex items-center gap-4 bg-white/50 p-3 rounded-2xl border border-white/40">
                                                {/* Hidden Native Picker Trigger */}
                                                <div className="relative group">
                                                    <div
                                                        style={{ backgroundColor: activeTheme.color }}
                                                        className="w-12 h-12 rounded-xl shadow-lg cursor-pointer border-2 border-white flex items-center justify-center transition-transform hover:scale-105 active:scale-95"
                                                ...
                                                <div className="flex flex-col flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[13px] font-black text-gray-800 tracking-tight uppercase">{activeTheme.color}</span>
                                                        <button
                                                            className="text-[10px] font-black text-pink-500 uppercase tracking-widest hover:opacity-70 transition-opacity"
                            ...
                            <div className="flex gap-3 mt-4 pt-4 border-t border-gray-100">
                                <Button
                                    ...
                                    className="!bg-white/50 !text-white-600 hover:!bg-white/80 !py-4 !rounded-[20px] font-black text-xs uppercase tracking-widest flex-1 border border-white/20 shadow-none"
                                >
                                    Hủy
                                </Button>
                                <Button
            ...
            {!isReadOnly && selectorAnchor && createPortal(
                <div className="fixed inset-0 z-[10001] md:bg-transparent bg-black/40 backdrop-blur-sm md:backdrop-blur-none flex items-end md:items-start transition-all" onClick={() => setSelectorAnchor(null)}>
                    <div
                        ref={selectorRef}
                        className={`
                            bg-white/95 backdrop-blur-3xl border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.2)] p-2 animate-slide-up md:animate-fade-in overflow-hidden
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-4 w-full p-4 md:p-3 rounded-2xl transition-all duration-200 group ${status === opt.id ? 'bg-pink-50' : 'hover:bg-gray-50'}`}
                                >
                                    ...
                                    <div className="flex flex-col items-start text-left">
                                        <span
                                            ...
                                            className={`text-[15px] md:text-[13px] font-black tracking-tight ${status === opt.id ? '' : 'text-gray-800'}`}
                                        >
                                            {opt.label}
                                        </span>
                                        <span className="text-[11px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mt-1">
                                            {opt.sub}
                                        </span>
                                    </div>
                                    {status === opt.id && (
