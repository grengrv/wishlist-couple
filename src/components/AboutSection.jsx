...
export default function AboutSection() {
  ...
  return (
    <section className="mb-12 relative overflow-hidden bg-white/60 backdrop-blur-xl rounded-3xl border border-pink-100 shadow-[0_8px_30px_rgb(236,72,153,0.08)] transition-all duration-300 hover:shadow-[0_8px_30px_rgb(236,72,153,0.15)] group">
      {/* Lớp nền gradient mờ ảo (Soft background blur) */}
      <div className="absolute inset-0 bg-gradient-to-br from-pink-50/40 via-transparent to-rose-50/40 pointer-events-none" />
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-pink-100/80 text-pink-500 transition-transform duration-300 group-hover:scale-110 group-hover:bg-pink-200">
        ...
        <div className={`flex items-center justify-center w-8 h-8 rounded-full transition-colors duration-300 ${isOpen ? 'bg-pink-100' : 'bg-transparent group-hover:bg-pink-50'}`}>
          <svg
      ...
      <div className={`grid transition-all duration-500 ease-in-out ${isOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
        <div className="overflow-hidden">
          <div className="px-6 pb-6 sm:px-8 sm:pb-8 pt-0">
            ...
            <p
              className={`text-[15px] sm:text-base leading-relaxed text-gray-600 border-l-[3px] border-pink-300/60 pl-4 sm:pl-5 transition-all duration-500 delay-75 transform ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0"}`}
            >
